...
class Pileup():    
    ...
    def __init__(self, sep_id = "", mode = "median", binning = 1000, cool_name = ""):
        self._sep_id = sep_id
    ...
    def add_submatrix(self, window, submatrix):
        ...
        with self.pileup_lock:
            if window in self._pileup_matrices:
                self._pileup_matrices[window].append(submatrix.flatten())
                self._nb_matrices[window] += 1
            else:
                self._pileup_matrices[window] = [submatrix.flatten()]
                self._nb_matrices[window] = 1
