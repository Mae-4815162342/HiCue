...
@click.option('-p', '--padding', type=float, default=1.0, help="Padding ratio to apply to each region. For instance, a padding of 0.5 will add on each side of an extracted region a padding of 0.5 x region_size. Default value: 1.0.")
@click.option('-s', '--min_region_size', type=int, default=20000, help="Minimal region size in bp. WARNING: regions with small sizes in large binning size might strongly bias any pileup result. Default value: 20000")
@click.option('-e', '--expected_sizes', type=INT_LIST, default="51", help="Expected sizes for resizing in pixels. Several values can be inputed as a comma-separated list. Default value: [51].")
@click.option('--gff', type=GFF_FILE, help="Gff file provided for the position file automatic annotation if the file is a bed2d. The positions considered for pileup are all the genes contained in the bed2d files.")
@click.option('--tracks', type=TRACK_FILE, help="Track file provided for the position file automatic annotation if the file is a bed2d. The positions considered for pileup are all the genes contained in the bed2d files.")
...
@click.option('--threads', type=int, default=8, help="Number of threads used by each multithreaded worker type. Default: 8.")
...
@click.option('--random_jitter', type=int, default=0, help="Allowed variation of the distance between two random loci in bp compared to the original distance value between pairs of positions. The distance will be selected in the [distance - random_jitter: distance + random_jitter] intervale.")
...
@click.option('--display_log/--no-display_log', default=True, help="Display in log.")
...
def regions(ctx, outpath, positions, cool_files, **params):
        ...
        h.regions(cool_files, positions, outpath, log = log, **params)
