...
@click.option('--tracks', type=GFF_FILE, help="Gff file provided for the position file automatic annotation if the file is a bed2d. The positions considered for pileup are all the genes contained in the bed2d files. For more options, use the hicue annotate command.") # TODO re-write
...
@click.option('-t', '--threads', type=int, default=8, help="Number of threads used by each multithreaded worker type.")
@click.option('-c', '--cpu', type=int, default=1, help="Number of available CPUs.")
...
@click.pass_context
def extract_opti(ctx, outpath, positions, cool_files, **params):
        ...
        h.extract_opti(cool_files, positions, outpath, log = log, **params)
